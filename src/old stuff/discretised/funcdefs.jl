...
function readdata(path)
	...
	# below works for file "observations.csv"
	# y = []
	# for r in eachrow(dat)
	# 	if r[:obsscheme]=="obs1"
...
struct DF
	...
	η::Vector{Float64}  # pars for including logradiatioin and temperature
	lrad_temp::Matrix{Float64}  # first column contains lograd; second column temperature
end



parameters(𝒫::DF) = (𝒫.α, 𝒫.ξ, 𝒫.σ2, 𝒫.ψ, 𝒫.η)
...
x:: Float64
Returns basic shape evaluated at x
"""
function ϕ0(x)
	x = 3.0x
	if 0.0<= x<= 1.0
...
if implicit
    ...
    a(k,𝒫) = k==0 ?  (@SVector [0.0]) :   (@SVector [A(k,𝒫)[1,1] * 𝒫.α * μ(𝒫.t[k+1],𝒫.ξ, 𝒫.J) * 𝒫.Δ[k]  + dot(𝒫.lrad_temp[k,:],𝒫.η) ])
else
    A(k,𝒫) = k==0 ?  SMatrix{1,1}([1.0]) : SMatrix{1,1}( [1.0 - 𝒫.α * 𝒫.Δ[k]] )
    a(k,𝒫) = k==0 ?  (@SVector [0.0]) :   (@SVector [𝒫.α *  μ(𝒫.t[k+1],𝒫.ξ) * 𝒫.Δ[k]   ])  # FIXME to include lograd/temp
end
...
function update_α(𝒫, x, acc, prior_α ; propσ=0.1)
	...
	𝒫ᵒ = DF(αᵒ, 𝒫.ξ, 𝒫.σ2, 𝒫.ψ, 𝒫.t, 𝒫.Δ, 𝒫.typeobs, 𝒫.J, 𝒫.η, 𝒫.lrad_temp)
...
function update_ξ(𝒫, x; β=0.01)
	...
	for i ∈ 2:n
		...
		zi = zi_ * ᾱ[i-1]
		#zi = zi_ .* vcat( fill(ᾱ[i-1],nb), fill(1.0,2) )          # a wild guess
		Z += zi * zi'
		v += U[i-1] * zi
	end
	P = PDMat(Symmetric(Z/𝒫.σ2 + β*I))
	ν = v/𝒫.σ2
...
function mcmc(𝒫, y; ITER = 1000, propσ=0.2, print_skip=100,
	...
	for it ∈ 2:ITER
		...
		𝒫 = DF(𝒫.α, 𝒫.ξ,  σ2, ψ, 𝒫.t, 𝒫.Δ, 𝒫.typeobs, 𝒫.J, 𝒫.η, 𝒫.lrad_temp)

		α, acc = update_α(𝒫, xs, acc, prior_α; propσ=propσ)
		𝒫 = DF(α, 𝒫.ξ, 𝒫.σ2, 𝒫.ψ, 𝒫.t, 𝒫.Δ, 𝒫.typeobs, 𝒫.J, 𝒫.η, 𝒫.lrad_temp)

		# ξ = update_ξ(𝒫, xs)
		# 𝒫 = DF(𝒫.α, ξ, 𝒫.σ2, 𝒫.ψ, 𝒫.t, 𝒫.Δ, 𝒫.typeobs, 𝒫.J, 𝒫.η, 𝒫.lrad_temp)

		ξ, η = update_ξ(𝒫, xs)
		𝒫 = DF(𝒫.α, ξ, 𝒫.σ2, 𝒫.ψ, 𝒫.t, 𝒫.Δ, 𝒫.typeobs, 𝒫.J, η, 𝒫.lrad_temp)
